...
impl GameHub {
    ...
    pub async fn join_room(
        ...
        input: JoinRoomIn,
        sender: ClientSender,
    ) -> Result<RoomStateOut, CustomError> {
    ...
    pub async fn leave_room(&self, room_code: &str, user_id: i64) -> Option<RoomStateOut> {
        let mut rooms = self.rooms.lock().await;
    ...
    pub async fn set_ready(
        ...
        let m = room
            .members
            .get_mut(&user_id)
            .ok_or_else(|| CustomError::BadRequest("用户不在房间中".into()))?;
        m.ready = ready;
    ...
    pub async fn start_game(
        ...
        user_id: i64,
        game_key: String,
    ) -> Result<(RoomStateOut, Vec<(i64, RoleRevealOut)>), CustomError> {
        let mut rooms = self.rooms.lock().await;
        let room = rooms
            .get_mut(room_code)
            .ok_or_else(|| CustomError::BadRequest("房间不存在".into()))?;
        ...
        let wolf_user_ids: Vec<i64> = roles
            .iter()
            .filter_map(|(uid, role)| {
                if *role == Role::Wolf {
                    Some(*uid)
        ...
        for (to_user_id, role) in roles {
            let wolf_ids_for_user = match role {
                Role::Wolf => wolf_user_ids
                    .iter()
                    .copied()
                    .filter(|id| *id != to_user_id)
                    .collect(),
    ...
    pub async fn broadcast(
        ...
        from_user_id: i64,
        text: String,
    ) -> Result<Vec<ClientSender>, CustomError> {
        let mut rooms = self.rooms.lock().await;
        let room = rooms
            .get_mut(room_code)
            .ok_or_else(|| CustomError::BadRequest("房间不存在".into()))?;
        ...
        let msg = WsInboundEnvelope {
            r#type: "broadcast".into(),
            payload,
        };
        let txt = serde_json::to_string(&msg).unwrap_or_else(|_| {
            "{\"type\":\"error\",\"payload\":{\"message\":\"serialize error\"}}".into()
        });
    ...
    pub async fn forward_to_user(
        ...
        let sender = room
            .senders
            .get(&to_user_id)
            .ok_or_else(|| CustomError::BadRequest("目标用户不在房间".into()))?;
        let _ = sender.send(msg.to_string());
        Ok(())
    }

    pub async fn broadcast_room_state(&self, room_code: &str) -> Result<(), CustomError> {
...
pub struct Room {
    ...
    pub senders: HashMap<i64, ClientSender>,
    pub game: Option<GameState>,
    pub broadcast_rate_limit: HashMap<i64, i64>,
}
...
impl Room {
    fn to_out(&self) -> RoomStateOut {
        ...
        RoomStateOut {
            ...
            status: self.status,
            members,
            game: self.game.clone().map(|g| GameOut {
                key: g.key,
...
pub type ClientReceiver = tokio::sync::mpsc::UnboundedReceiver<String>;
...
pub fn spawn_outbound_forwarder(sink: WsSink, mut rx: ClientReceiver) {
...
pub async fn handle_frame(
    ...
    let text = match frame {
        Frame::Text(bytes) => String::from_utf8(bytes.to_vec())
            .map_err(|_| CustomError::BadRequest("Invalid UTF-8".into()))?,
    ...
    match env.r#type.as_str() {
        ...
        "leave_room" => {
            ...
            let _ = context
                .hub
                .leave_room(&input.room_code, input.user_id)
                .await;
            context
                .hub
                .broadcast_room_state(&input.room_code)
                .await
                .ok();

            {
                let mut st = conn_state.borrow_mut();
                if st.room_code.as_deref() == Some(&input.room_code)
                    && st.user_id == Some(input.user_id)
                {
                    st.room_code = None;
                    st.user_id = None;
        ...
        "set_ready" => {
            ...
            let _state = context
                .hub
                .set_ready(&input.room_code, input.user_id, input.ready)
                .await?;
            context.hub.broadcast_room_state(&input.room_code).await?;
            Ok(None)
        }
        "start_game" => {
            ...
            for (to_user_id, reveal) in reveals {
                ...
                let msg = serde_json::to_string(&env).unwrap_or_else(|_| {
                    "{\"type\":\"error\",\"payload\":{\"message\":\"serialize error\"}}".into()
                });
                let _ = context
                    .hub
                    .forward_to_user(&input.room_code, to_user_id, &msg)
                    .await;
            }
        ...
        "broadcast" => {
            ...
            let _ = context
                .hub
                .broadcast(&input.room_code, input.user_id, input.text)
                .await?;
            Ok(None)
        }
        "role_reveal" => {
        ...
        other => {
            let env = WsInboundEnvelope {
                r#type: "error".into(),
                payload: json!({"message": format!("Unknown type: {other}")}),
            };
            let txt = serde_json::to_string(&env).unwrap_or_else(|_| {
                "{\"type\":\"error\",\"payload\":{\"message\":\"unknown\"}}".into()
            });
            Ok(Some(Message::Text(ByteString::from(txt))))
        }
    }
...
pub fn validate_room_code(code: &str) -> Result<(), CustomError> {
    if code.len() != 6 {
...
pub fn wolf_count(total: usize) -> usize {
    match total {
        0 => 0,
        1..=5 => 1,
        6..=8 => 2,
...
pub fn deal_witness_werewolf_roles(user_ids: &[i64]) -> Vec<(i64, Role)> {
