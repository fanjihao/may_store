use crate::{
    ...
    models::{
        users::UserToken,
        wishes::{
            WishClaimCreateInput,
            WishClaimOut,
            WishClaimStatusEnum,
            WishClaimUpdateInput,
            WishStatusEnum,
        },
    },
    AppState,
...
use chrono::Utc;
use ntex::web::{ types::{ Json, State, Query }, HttpResponse, Responder };
use sqlx::Row;
use std::sync::Arc;
...
#[utoipa::path(
    ...
    path = "/wish_claims",
    tag = "心愿",
    request_body = WishClaimCreateInput,
    responses((status = 201, body = WishClaimOut))
)]
pub async fn get_claim(
    _: UserToken,
    state: State<Arc<AppState>>,
    data: Query<WishClaimCreateInput>
...
WHERE
  wish_id = $1"
    ...
    let claim_status: WishClaimStatusEnum = r.get("status");
...
pub async fn claim_wish(
    ...
    let wish_row = sqlx
        ::query(
            "SELECT wish_id, wish_name, wish_cost, status, created_by, created_at, updated_at FROM wishes WHERE wish_id=$1 FOR SHARE"
        )
        .bind(data.wish_id)
    ...
    let claim_row = sqlx
        ::query(
            "INSERT INTO wish_claims (wish_id, user_id, cost, status, remark) VALUES ($1,$2,$3,'PROCESSING',$4) RETURNING id, wish_id, user_id, cost, status, remark, fulfill_at, created_at, updated_at"
        )
        .bind(data.wish_id)
        .bind(user_token.user_id)
        .bind(wish_cost)
...
pub async fn update_wish_claim(
    ...
    let row = sqlx
        ::query(
            "SELECT id, wish_id, user_id, cost, status, remark, fulfill_at, created_at, updated_at FROM wish_claims WHERE id=$1 FOR UPDATE"
        )
        .bind(data.claim_id)
        .fetch_optional(&mut *tx).await?;
    let Some(r) = row else {
    ...
    let current_status: WishClaimStatusEnum = r.get("status");
    ...
    match data.to_status {
        WishClaimStatusEnum::DONE => {
            sqlx
                ::query(
                    "UPDATE wish_claims SET status='DONE', fulfill_at=NOW(), remark=$2, updated_at=NOW() WHERE id=$1"
                )
                .bind(data.claim_id)
                .bind(&data.remark)
                .execute(&mut *tx).await?;
            ...
            tokio::spawn(async move {
                if
                    let Err(e) = crate::services::notifications::push_order_status(
                        wish_id,
                        pool_clone.clone()
                    ).await
                {
        ...
        WishClaimStatusEnum::CANCELLED => {
            sqlx
                ::query(
                    "UPDATE wish_claims SET status='CANCELLED', remark=$2, updated_at=NOW() WHERE id=$1"
                )
                .bind(data.claim_id)
                .bind(&data.remark)
                .execute(&mut *tx).await?;
