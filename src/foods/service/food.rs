...
use crate::foods::models::food::{
    ...
    FoodCreateInput, FoodFilterQuery, FoodOut, FoodRecord, FoodStatusEnum,
    FoodUpdateInput, FoodWithStatsRecord, MarkTypeEnum, SubmitRoleEnum,
};
...
pub async fn create_food(
    ...
    let marks: Vec<String> = sqlx::query("SELECT mark_type::text FROM user_food_mark WHERE user_id=$1 AND food_id=$2")
        .bind(token.user_id as i64)
        .bind(rec.food_id)
        .fetch_all(&mut *tx)
        .await?
...
pub async fn get_foods(
    ...
    if let Some(kw) = &q.keyword {
        qb.push(" AND f.food_name ILIKE '%' || ").push_bind(kw).push(" || '%'");
    }
    ...
    qb.push(" ORDER BY f.created_at DESC, f.food_id DESC LIMIT ").push_bind(limit + 1);
    ...
    Ok(CursorPage { items, next_cursor, has_more, total: None })
}
...
pub async fn get_food_detail(
    ...
    let tag_row: Option<TagRecord> = if let Some(tid) = rec.tag_id {
        sqlx::query_as("SELECT * FROM tags WHERE tag_id=$1").bind(tid).fetch_optional(db).await?
    } else {
        None
    };
...
pub async fn update_food(
    ...
    if !is_admin && matches!(rec.submit_role, SubmitRoleEnum::OrderingApply) && rec.created_by == uid && data.apply_status.is_some() {
        return Err(CustomError::BadRequest("禁止自审".into()));
    }

    if !is_admin && matches!(rec.submit_role, SubmitRoleEnum::OrderingApply) && data.apply_status.is_some() && !is_receiving {
        return Err(CustomError::BadRequest("仅接单角色可审核".into()));
    }

    if let Some(name) = &data.food_name { rec.food_name = name.clone(); }
    if let Some(photo) = &data.food_photo { rec.food_photo = Some(photo.clone()); }
    if let Some(ing) = &data.ingredients { rec.ingredients = Some(ing.clone()); }
    if let Some(st) = &data.steps { rec.steps = Some(st.clone()); }
    if let Some(r) = &data.apply_remark { rec.apply_remark = Some(r.clone()); }
    if let Some(status) = data.food_status { rec.food_status = status; }
    if let Some(app_status) = data.apply_status { rec.apply_status = app_status; }
    if let Some(tid) = data.tag_id { rec.tag_id = Some(tid); }
    ...
    let tag_row: Option<TagRecord> = if let Some(tid) = rec.tag_id {
        sqlx::query_as("SELECT * FROM tags WHERE tag_id=$1").bind(tid).fetch_optional(&mut *tx).await?
    } else {
        None
    };
...
pub async fn mark_food(db: &PgPool, user_id: i64, food_id: i64, mark_type: MarkTypeEnum) -> Result<(), CustomError> {
...
pub async fn unmark_food(db: &PgPool, user_id: i64, food_id: i64, mark_type: MarkTypeEnum) -> Result<(), CustomError> {
...
pub async fn get_marked_foods(
    ...
    for rec in rows {
        ...
        items.push(FoodOut::from_with_stats(rec, tag_row, vec![MarkTypeEnum::LIKE]));
    }
