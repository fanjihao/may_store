...
use crate::foods::models::tag::{FoodTagOut, TagCreateInput, TagRecord, TagUpdateInput, BatchTagSortInput};
...
pub async fn get_tags(db: &PgPool, query: &FoodFilterQuery) -> Result<Vec<FoodTagOut>, CustomError> {
...
pub async fn delete_tag(db: &PgPool, id: i64) -> Result<(), CustomError> {
    ...
    // check usage
    let count: (i64,) = sqlx::query_as("SELECT COUNT(*) FROM foods WHERE tag_id=$1 AND is_del=0")
        .bind(id)
        .fetch_one(&mut *tx)
        .await?;
    if count.0 > 0 {
...
pub async fn update_tags_sort(db: &PgPool, data: &BatchTagSortInput) -> Result<(), CustomError> {
