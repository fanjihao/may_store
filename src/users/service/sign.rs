...
impl SignService {
    pub async fn daily_checkin(mut user_token: UserToken, state: &AppState) -> Result<DailyCheckinOut, CustomError> {
    ...
    pub async fn get_sign_info(user_id: i64, state: &AppState) -> Result<SignInfoResponse, CustomError> {
    let db = &state.db_pool;
    let today = Local::now().date_naive();
