...
use crate::{
    ...
    users::models::user::{
        ...
        ProfileUpdateInput, RegisterInput, RoleSwitchInput, RoleSwitchResult, 
...
impl UserService {
    ...
    let (pwd_hash, algo) = Self::hash_password(&data.password).map_err(|e| CustomError::internal(e))?;
    ...
    pub async fn get_current_info(user_id: i64, state: &AppState) -> Result<UserPublic, CustomError> {
    ...
    pub async fn get_user_info(username: &str, state: &AppState) -> Result<UserPublic, CustomError> {
    let db = &state.db_pool;
    let rec = sqlx::query_as::<_, UserRecord>(r#"
    ...
    pub async fn is_register(username: &str, state: &AppState) -> Result<IsRegisterResponse, CustomError> {
    ...
    pub async fn change_info(data: ProfileUpdateInput, state: &AppState) -> Result<UserPublic, CustomError> {
    let db_pool = &state.db_pool;
    ...
    if let Some(new_pwd) = &data.new_password {
        ...
        let (hash, algo) = Self::hash_password(new_pwd).map_err(|e| CustomError::internal(e))?;
    ...
    pub async fn switch_role(user_id: i64, body: RoleSwitchInput, state: &AppState) -> Result<RoleSwitchResult, CustomError> {
    let db = &state.db_pool;
