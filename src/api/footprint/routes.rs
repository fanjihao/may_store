...
use crate::domain::footprint::{CheckPermissionResponse, FootprintOverview, RecordCreateInput, RecordGroup, RecordOut, RecordQuery, RecordUpdateInput, SubmitRecordResponse};
...
pub fn configure(cfg: &mut ServiceConfig) {
    cfg.service(
        web::scope("/footprint")
            ...
            .route("/check-permission", web::get().to(check_permission))
            .route("/groups", web::get().to(get_groups))
            .route("/records", web::get().to(get_records))
            .route("/records", web::post().to(submit_record))
...
pub async fn get_record(
    ...
    let res =
        FootprintService::get_record(&state.db_pool, token.user_id, group_id, id.into_inner())
            .await?;
    Ok(HttpResponse::Ok().json(&res))
}
...
pub async fn expand_capacity(
    ...
    Ok(
        HttpResponse::Ok()
            .json(&serde_json::json!({ "status": "ok", "newCapacity": new_capacity })),
    )
}
