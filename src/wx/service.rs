...
use crate::{
    errors::CustomError,
    private::{APP_ID, APP_SECRET, OFFCIAL_APP_ID, OFFCIAL_APP_SECRET},
};
...
impl WxService {
    ...
    pub async fn fetch_set_access_token() -> Result<(), CustomError> {
        if let Some(_access_token) = Self::get_access_token().await {
            Ok(())
        } else {
            ...
            match response_json {
                Ok(obj) => {
                    ...
                    Self::set_access_token(token.to_string(), time::Duration::from_secs(7200))
                        .await;
                    Ok(())
                }
    ...
    pub async fn fetch_set_mp_token() -> Result<(), CustomError> {
        if let Some(_access_token) = Self::get_mp_token().await {
            Ok(())
        } else {
            let body = reqwest::get(
                "https://api.weixin.qq.com/cgi-bin/token?grant_type=client_credential&appid="
            ...
            match response_json {
                Ok(obj) => {
                    ...
                    Self::set_mp_token(token.to_string(), time::Duration::from_secs(7200)).await;
                    Ok(())
                }
                Err(_) => Err(CustomError::BadRequest(
    ...
    pub async fn wx_offical_create_menu() -> Result<String, CustomError> {
        ...
        let token = match token {
            Some(token) => token,
            None => {
                Self::fetch_set_access_token().await?;
                let new_token = Self::get_access_token().await.unwrap();
                new_token
            }
        };
