# may_store 协作型生活管理系统 —— 可上线级架构方案（全量版）
...
- **API 层**：RESTful/GraphQL/WebSocket，负责协议适配、参数校验、认证授权。
- **应用层**：编排业务用例，处理事务、幂等、调用领域服务、发布领域事件。
- **领域层**：核心业务逻辑（如订单状态机、经济联动、成就规则），聚合根、实体、值对象、领域服务。
- **事件层**：事件定义、事件发布、事件消费（Worker），实现解耦与异步处理。
- **基础设施层**：数据库、缓存、消息队列、外部服务适配器。

### 1.2 模块划分

- **user**：用户、认证、分组（group/association）
- **order**：订单（任务）、状态机、评价
- **economy**：point/diamond 体系、流水、联动机制
- **footprint**：足迹、容量、状态管理
- **achievement**：成就、规则、事件驱动
- **event**：事件定义、事件日志、Worker
- **utils/middleware**：通用工具、日志、鉴权等
...
- **状态机**：Created → Accepted → Completed/Timeout/Cancelled
- **并发安全**：乐观锁/条件更新防止重复接单
- **评价体系**：订单完成后可评价
- **催办/附件**：支持催办、上传附件
- **生命周期**：创建、接单、完成、超时、取消
...
- **双货币模型**：Point（个人付出）、Diamond（组资源）
- **联动机制**：行为→point，高质量关系→diamond
- **流水机制**：所有变动写入流水表，余额聚合
- **排行榜**：按point/diamond统计排行

### 2.4 足迹系统（footprint）

- **容量管理**：group级别容量，diamond扩容
- **状态管理**：draft/published/deleted，软删除
- **地理/图片**：支持地理位置、图片
- **与订单关联**：订单完成自动发布足迹

### 2.5 成就系统（achievement）
