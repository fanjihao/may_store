...
- `src/v3.sql` 是干净安装后的**最终结构权威来源**（全量建表脚本，带完整类型 / 索引 / 注释）。
- `migrations/` 是已有数据库的**唯一前向升级链路**，应用启动时由 SQLx 按版本执行。
- 凡是涉及数据库结构变更的工作（新建表、加列、改类型、改索引、新增 enum、加 / 改函数），**必须同时**：
  1. 更新 `src/v3.sql`，保证新装数据库直接得到最终结构；
  2. 新增 forward-only migration，保证已有数据库不清库即可升级。
- 禁止修改已经发布的 migration；修正只能追加新版本。`src/utils/migrations_legacy/` 仍仅供历史参考，严禁执行。
- 新增接口、改 SQL 查询时：
